...
source("OutlierTreatment.R") # imports custom function for detecting extreme outliers and replacing.
```
...
tswide <- main %>% mutate(tsmonth = yearmonth(month2)) %>%
...
tslong <- tswide %>% dplyr::select(!c(tsmonth, bedocc:mrsa_all_bsi)) %>%
  pivot_longer(
...
maints2 <- maints %>% mutate(across(abhr:rectalswab, OutlierTreatment(.,freq=12,start.date=c(2008,1))))

ggplot (maints, aes(x=tsmonth, y=abhr))+
  geom_line(linetype="dotdash", colour="red")+
  geom_line(data=maints2, aes(x=tsmonth,y=abhr))
...
# For 
acf(maints2$all_gn_bsi, lag=24)

maints2 %>% dplyr::select(maints2$abhr)  %>%
  gg_tsdisplay(difference(abhr), plot_type='partial')

gg_tsdisplay(maints2, all_gn_bsi,  "partial")
all_gn_bsi_d1 <- diff(maints2$all_gn_bsi, differences = 1) %>% as_tsibble()

gg_tsdisplay(difference(maints2$all_gn_bsi)), plot_type='partial')


all_gn_bsi_tsd <- maints2 %>%
  model(
    STL(all_gn_bsi ~ trend(window = 7) +
                   season(window =13),
    robust = TRUE)) %>%
  components() %>%
  autoplot()
...
maints2 %>%
  model(STL(all_gn_bsi ~ trend(window = 10))) %>%
  components()
