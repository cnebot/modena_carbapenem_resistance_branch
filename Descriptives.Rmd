...
main <- cbind(date,main) %>% dplyr::select(!c(months, study_m))
...
wide_clean <- tswide %>% as_tibble %>% dplyr::select(date, obd, bedocc,
...
tmp <-main %>% dplyr::select(!c(date, obd,bedocc,
abhr:fluoroquinolones))
tswide_clean <- cbind(tmp, newabx.matrix) %>% as_tsibble(index=date)
...
ggplot(orgpredsed, aes(year, fit, color=Population, fill=Population))+
  ...
  scale_x_continuous ("Year", breaks=seq(2008,2018,2))+
  scale_y_continuous(name="% Carbapenem resistant (I/R)",
             labels=scales::percent_format(accuracy=1),
             limits=c(0,NA))+
