...
source("extract_impulse.R") # function to extract coefficients for impulse response functions
...
First we VAR gram negatve bsi, ecoliccs and ecoli3gcr with each ABX
...
var.fit <- vars::VAR(matrix,lag.max = 4, ic="AIC")
...
causality<- list(cause.y=vars::causality(var.fit,  cause=names[1],boot=TRUE, boot.runs=1000)$Granger, cause.series=vars::causality(var.fit, cause=names[2], boot=TRUE, boot.runs=1000)$Granger)
...
assign(paste0(c("var",colnames(pre_icarba)[n],colnames(pre_icarba)[j]), sep=".", collapse = ""),var)
 }
}

```

Direction of interactions between populations
...
# Attempt to add ggplot function
...
ext<- extract_varirf(var[["impulse.response"]]) %>% 
  pivot_longer(!period, 
...
assign(paste0(c("var",colnames(pre_icarba)[n],colnames(pre_icarba)[j]), sep=".", collapse = ""),var)
assign(paste0(c("ext",colnames(pre_icarba)[n],colnames(pre_icarba)[j]), sep=".", collapse = ""), ext)
 }
}


## GGplot from "ext" object function

irfplot <- function(x) {
  ggplot(x, 
       aes(x=period, y=irf, color=y, fill=y))+
  geom_line()+
...
# Make final plots
p1<-irfplot(ext.gmneg_all_bsi.fq.)
p2<-irfplot(ext.gmneg_all_bsi.carbapenem.)
p3<-irfplot(ext.ecoliccs_all_bsi.fq.)
p4<-irfplot(ext.ecoliccs_all_bsi.carbapenem.)
p5<-irfplot(ext.ecoli3gcr_all_bsi.fq.)
p6<-irfplot(ext.ecoli3gcr_all_bsi.carbapenem.)

p1+p2+plot_layout(guides="collect")

irfplot(ext.gmneg_all_bsi.all_monitored.)
