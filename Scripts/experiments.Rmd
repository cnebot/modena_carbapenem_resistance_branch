...
tswide_clean %>%
  model(
    STL(gmneg_all_clinical ~ trend(window = 21) +
                   season(window = 5),
    robust = TRUE)) %>%
  components() %>%
  autoplot()
...
ts_decomp <- ts_decomp %>% unnest(decs)
ts_decomp$decs <-map(ts_decomp$decs, ~.x[[1]]$x)
...
lambda<-guerrero(tswide_clean$noncarb, .period=12L)
lambda2<-guerrero(tswide_clean$gmneg_all_clinical, .period=12L)
lambda3<-guerrero(tswide_clean$carbapenem, .period=12L)
lambda4<-guerrero(tswide_clean$threegc, .period=12L)

#### create new box-cox transformed variables
tswide_clean$noncarb_bc <- 
  (((tswide_clean$noncarb)^lambda)-1)/lambda
tswide_clean$gmneg_all_clinical_bc <-
  (((tswide_clean$gmneg_all_clinical)^lambda2)-1)/lambda2
tswide_clean$carbapenem_bc <-
  (((tswide_clean$carbapenem)^lambda3)-1)/lambda3
tswide_clean$threegc_bc <- 
  (((tswide_clean$threegc)^lambda4)-1)/lambda4
...
############ Non-carbapenem Abx
ts_p3 %>% gg_tsdisplay(difference(noncarb), "partial")
ts_p3 %>% gg_tsdisplay(difference(noncarb_bc), "partial")
ts_p3 %>% gg_tsdisplay(difference(noncarb,12) %>%
                         difference(), "partial")

# Fit best manual and two auto ARIMAs
fit <- ts_p3 %>% model(
  manual=ARIMA(noncarb~ pdq(3,1,0)+PDQ(0,0,0)),
  manual2=ARIMA(noncarb~ pdq(1,0,0)+PDQ(0,0,0)),
  auto = ARIMA(noncarb~pdq(1,1,0)))
  auto2 = ARIMA(noncarb, stepwise=FALSE))

fit2<- auto.arima(ts_p3$noncarb, d=0, D=0, max.p=4, max.q=0, max.P=0, max.Q=0, start.p=4, ic="aic", allowdrift=TRUE)

# see results
glance(fit) # compare BIC, AICc etc
fit %>% select(manual) %>% report() # see manual ARIMA
fit %>% select(manual) %>% gg_tsresiduals() #see manual ARIMA residuals
augment(fit) %>%
  filter(.model=='manual') %>%
  features(.innov, ljung_box, lag = 10, dof = 3) #Pormanteau test of residuals

fit %>% select(auto) %>% report() # see first auto ARIMA
fit %>% select(auto) %>% gg_tsresiduals() # see first auto ARIMA residuals
augment(fit) %>%
  filter(.model=='auto') %>%
  features(.innov, ljung_box, lag = 10, dof = 3) #Pormanteau test of residuals

fit %>% select(auto2) %>% report() #see second auto ARIMA
fit %>% select(auto2) %>% gg_tsresiduals() #see second auto ARIMA resid
augment(fit) %>%
  filter(.model=='auto2') %>%
  features(.innov, ljung_box, lag = 10, dof = 3) #Pormanteau test of residuals
...
ts_p3 %>%
  autoplot(noncarb) +
  autolayer(fit %>% select(auto) %>% fabletools::forecast(h = 52),
    colour = "blue", alpha=0.5, level = 95)+
  autolayer(fit %>% select(auto)%>% fabletools::forecast(h = 52),
    colour = "green", alpha=0.5, level = 95)+
    autolayer(fit %>% select(manual)%>% fabletools::forecast(h = 52),
...
ts_p3 %>% gg_tsdisplay(difference(carbapenem), "partial")
ts_p3 %>% gg_tsdisplay(difference(carbapenem_bc), "partial")
ts_p3 %>% gg_tsdisplay(difference(threegc), "partial")
ts_p3 %>% gg_tsdisplay(difference(threegc_bc), "partial")
...
ts_p3 %>% features(noncarb_bc, unitroot_kpss)
ts_p3  %>% features(noncarb_bc, unitroot_ndiffs) 
ts_p3  %>% features(noncarb_bc, unitroot_nsdiffs,.period=12) 

ts_p3 %>% gg_tsdisplay(difference(crpa_hca_clinical), "partial") 
...
arimax_gmneg <- ts_p3 
lambdafq<-guerrero(arimax_gmneg$fq, .period=12L)

tswide_clean$crpa_conothca_clinical_bc <- (((tswide_clean$crpa_conothca_clinical)^lambda)-1)/lambda


CCF(ts_p3, difference(threegc_bc), difference(gmneg_all_clinical), 
    lag_max=24) %>% autoplot(level=95)

CCF(ts_p3, difference(ecoli_esbl_all_clinical), 
    difference(noncarb_bc), lag_max=24) %>% autoplot(level=95)


fit_determ<-ts_p3 %>%
  model(deterministic = ARIMA(threegc~1+trend()+ pdq(d = 0)))
report(fit_determ)

fit_stochastic <-ts_p3 %>%
  model(stochastic = ARIMA(threegc~ pdq(d = 1)+PDQ(D=1)))
report(fit_stochastic)

fit_auto<-ts_p3 %>%
  model(auto=ARIMA(threegc))
fit_auto %>% report()

ts_p3 %>%
  autoplot(threegc) +
  autolayer(fit_stochastic %>% fabletools::forecast(h = 52),
    colour = "#0072B2", alpha=0.5, level = 95)+
  autolayer(fit_determ %>% fabletools::forecast(h = 52),
    colour = "#D55E00", alpha=0.5, level = 95)+
    autolayer(fit_auto %>% fabletools::forecast(h = 52),
    colour = "red", alpha=0.2, level = 95)

fits<-ts_p3 %>%
  # Restrict data so models use same fitting period
  mutate(noncarb = c(NA, NA, NA,NA,NA,NA, ts_p3$noncarb[7:80])) %>%
  # Estimate models
  model(
    lag0 = ARIMA(noncarb ~ pdq(d = 1) + difference(gmneg_all_clinical)),
    lag1 = ARIMA(noncarb ~ pdq(d = 1) +
                   difference(gmneg_all_clinical)+
                   lag(difference(gmneg_all_clinical))),
    lag2 = ARIMA(noncarb ~ pdq(d = 1) +
                   difference(gmneg_all_clinical)+
                   lag(difference(gmneg_all_clinical))+
                   lag(difference(gmneg_all_clinical,2))),
    lag3 = ARIMA(noncarb ~ pdq(d = 1) +
                   difference(gmneg_all_clinical)+
                   lag(difference(gmneg_all_clinical))+
                   lag(difference(gmneg_all_clinical,2))+
                   lag(difference(gmneg_all_clinical,3))),
    lag4 = ARIMA(noncarb ~ pdq(d = 1) +
                   difference(gmneg_all_clinical)+
                   lag(difference(gmneg_all_clinical))+
                   lag(difference(gmneg_all_clinical,2))+
                   lag(difference(gmneg_all_clinical,3))+
                   lag(difference(gmneg_all_clinical,4))),
    lag5 = ARIMA(noncarb ~ pdq(d = 1) +
                   difference(gmneg_all_clinical)+
                   lag(difference(gmneg_all_clinical))+
                   lag(difference(gmneg_all_clinical,2))+
                   lag(difference(gmneg_all_clinical,3))+
                   lag(difference(gmneg_all_clinical,4))+
                   lag(difference(gmneg_all_clinical,5))),
    lag6 = ARIMA(noncarb ~ pdq(d = 1) +
                   difference(gmneg_all_clinical)+
                   lag(difference(gmneg_all_clinical))+
                   lag(difference(gmneg_all_clinical,2))+
                   lag(difference(gmneg_all_clinical,3))+
                   lag(difference(gmneg_all_clinical,4))+
                   lag(difference(gmneg_all_clinical,5))+
                   lag(difference(gmneg_all_clinical,6))))

fitnoncarb <- ts_p3 %>%
  model(
    aicc = VAR(vars(gmneg_all_clinical, noncarb)),
    bic = VAR(vars(gmneg_all_clinical, noncarb) ~ AR(6), ic = "bic")
  )
fitnoncarb <- ts_p3 %>% model(VAR(vars(gmneg_all_clinical, noncarb) ~ AR(6), ic = "bic"))
...
We create three data sets representing periods before interventions 1 to 3 allowing characterisation oftemporal behaviour in time series before influence of each intervention.
...
tswide_clean %>% features(crkp_hca_clinical, unitroot_kpss)
tswide_clean %>% features(crkp_hca_clinical, unitroot_ndiffs) 
tswide_clean %>% features(crkp_hca_clinical, unitroot_nsdiffs,.period=12) 

#### Examine need for a Box-Cox data transformation to reduce heteroskedasticity (i.e variability in the variance of the time-series)
lambda<-guerrero(tswide_clean$crkp_hca_clinical, .period=12L)

#### create new transformed variable
tswide_clean$crkp_hca_clinical_bc <- (((tswide_clean$crkp_hca_clinical)^lambda)-1)/lambda
...
arima_crkp_ho <- tswide_clean %>% model(
  arima_crkp_hca_101 = ARIMA(crkp_hca_clinical ~ pdq(1,0,1)),
  stepwise_crkp_ho = ARIMA(crkp_hca_clinical)) 

glance(arima_crkp_ho) %>% arrange(AICc) %>% dplyr::select(.model:BIC)
arima_crkp_ho  %>% dplyr::select(arima_crkp_hca_101) %>% gg_tsresiduals() # compare models
...
arima_crkp_hca_bc <- tswide_clean %>% model(
  arima_crkp_hca_bc_111 = ARIMA(crkp_hca_clinical_bc ~ pdq(1,1,1)),
  arima_crkp_hca_bc_101_100 = ARIMA(crkp_hca_clinical_bc ~ pdq(1,0,1) + PDQ(1,0,1)),
    arima_crkp_hca_bc_011_100 = ARIMA(crkp_hca_clinical_bc ~ pdq(0,1,1) + PDQ(1,0,0)),
  arima_crkp_hca_bc_101 = ARIMA(crkp_hca_clinical_bc ~ pdq(1,0,1)),
  arima_crkp_hca_bc_001 = ARIMA(crkp_hca_clinical_bc ~ pdq(0,0,1)),
  stepwise_crkp_hca_bc = ARIMA(crkp_hca_clinical_bc)) 

glance(arima_crkp_hca_bc) %>% arrange(AICc) %>% dplyr::select(.model:BIC)

arima_crkp_hca_bc  %>% dplyr::select(arima_crkp_hca_bc_011_100) %>% gg_tsresiduals() # compare models
arima_crkp_hca_bc %>% dplyr::select(stepwise_crkp_hca_bc) %>% gg_tsresiduals() # compare models

#### Check portmanteau tests of best fit option
augment(arima_crkp_hca_bc) %>% 
  filter(.model=='arima_crkp_hca_bc_011_100') %>%
...
## 4.1 Gram-Negative clinical infections and abx use
...
boxcox_fn <- function(x){
  lambda <-guerrero(x, .period=12L)
  paste0(x,"_bc")
...
CCF(ts_p3, difference(gmneg_all_clinical), 
    difference(`3gc`), lag_max=24) %>% autoplot(level=95)
