...
arima_crkp_hca <- tswide_clean %>% model(
  arima_crkp_hca_101 = ARIMA(crkp_hca_clinical ~ pdq(1,0,1)),
  stepwise_crkp_hca= ARIMA(crkp_hca_clinical)) 

glance(arima_crkp_hca) %>% arrange(AICc) %>% dplyr::select(.model:BIC)
arima_crkp_hca  %>% dplyr::select(arima_crkp_hca_101) %>% gg_tsresiduals() # compare models
arima_crkp_hca%>% dplyr::select(stepwise_crkp_hca) %>% gg_tsresiduals() # compare models
...
lambda<-guerrero(tswide_clean$gmneg_all_clinical, .period=12L)
lambda<-guerrero(tswide_clean$noncarb, .period=12L)

ts_p3$gmneg_all_clinical_bc <- (((ts_p3$gmneg_all_clinical)^lambda)-1)/lambda
ts_p3$noncarb_bc <-(((ts_p3$noncarb)^lambda)-1)/lambda
