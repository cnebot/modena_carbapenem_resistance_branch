...
# var.order<-vars::VARselect(matrix, lag.max=6)[["selection"]][1]

# var.fit <- vars::VAR(matrix, lag.max = as.integer(var.order))


var.order<-as.integer(vars::VARselect(matrix, lag.max=6)[["selection"]][1])
var.fit <- vars::VAR(matrix,p=var.order )
...
matrix<-cbind(x,y,z)
colnames(matrix)<-names
var.order=as.integer( VARselect(matrix, lag.max=6)[["selection"]][1])
var.fit <- VAR(matrix, lag.max =var.order)
